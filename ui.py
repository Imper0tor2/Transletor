...
class TranslatorWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.input_text.textChanged.connect(self.update_counter) #при каждом изменении текста запускать метод изменения счёчика
    ...
    def update_counter(self):
        if len(self.input_text.toPlainText()) > self.max_char:
            self.input_text.setText(self.input_text.toPlainText()[0:self.max_char]) # срезом обрезаем до 15 000 символов
